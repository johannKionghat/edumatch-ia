...
Le RGAA (Référentiel général d'amélioration de l'accessibilité) s'appuie sur
WCAG 2.1 niveau AA. Ce document décrit la procédure d'audit **manuel**, au
navigateur, qui referme ce que la suite automatisée
...
saut, landmarks uniques), chaque champ de saisie associé à un `<label>`,
zones dynamiques annoncées (`role="alert"`, `aria-live`), contraste calculé
exactement (formule WCAG) sur les couleurs déclarées dans `style.css`, focus
jamais supprimé sans remplacement visible, absence d'`innerHTML` (donc pas
de vecteur XSS par construction), syntaxe JavaScript valide.
...
Prérequis : lancer l'API (`make api`), ouvrir `http://localhost:8000/` dans
le navigateur audité.
...
- [ ] Après soumission du formulaire, le focus n'est pas perdu (il reste sur
      le bouton, ou se déplace vers un message d'erreur s'il y en a un).
...
- [ ] Cliquer « Comprendre cette recommandation » au clavier (`Entrée`)
      ...
      (curseur qui semble ne plus être nulle part visible).
...
- [ ] Aucun piège au clavier : on peut toujours ressortir d'un composant par
      `Tab` ou `Échap`.
...
- [ ] Le titre de la page est annoncé à l'ouverture (« Écran conseiller —
      EduMatch-IA »).
- [ ] Le lecteur annonce les landmarks (« bannière », « navigation »,
      « contenu principal », « pied de page ») en mode navigation par
      landmarks.
- [ ] Chaque champ du formulaire est annoncé avec son étiquette complète, et
      les groupes de boutons radio annoncent leur légende (« Type de
...
- [ ] Le tableau de contributions (panneau « Comprendre ») est annoncé comme
      un tableau, avec ses en-têtes de colonne lus avant chaque cellule
      (« Variable », « Effet », « Contribution »).
- [ ] Le message de confirmation d'un écartement (« Décision écartée
      enregistrée... ») est annoncé sans que l'utilisateur ait à déplacer le
      focus (`role="status"`, `aria-live="polite"`).
- [ ] Le statut « Mesuré » / « Indisponible » des débouches est lu en toutes
      lettres, pas seulement comme une pastille de couleur muette.
...
- [ ] Fenêtre réduite à 320 px de large (équivalent mobile) : le formulaire
      et les cartes de recommandation restent utilisables et lisibles.
...
- [ ] Simuler un daltonisme (outil navigateur ou extension) : les statuts
      ...
      seulement la couleur du badge — voir point automatisé correspondant).
- [ ] Vérifier au pixel, avec l'outil d'inspection du navigateur, que les
      ...
      thème sombre du système qui changerait le rendu).
